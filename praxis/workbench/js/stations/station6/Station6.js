...
(function () {
  ...
  // Shared HTML escaper: coerces non-strings (numbers, null) before escaping.
  function esc(s) {
    return window.PraxisExportUtils.escHtml(s);
  }
  ...
  function exportAnalysisPlan(cards, programmeName) {
    ...
    function buildTable(rows) {
      ...
      rows.forEach(function (card) {
        ...
        t += '<p style="margin:0 0 3pt;font-size:9pt;font-weight:bold;color:#64748B">LINKED INDICATORS</p>';
        var indText = (card.indicators || []).map(function (ind) {
          return (ind.code ? '[' + ind.code + '] ' : '') + (ind.name || '');
        }).join('; ');
        t += '<p style="margin:0 0 6pt;font-size:9pt;color:#374151">' + esc(indText || 'None linked') + '</p>';
        ...
        if (card.notes && String(card.notes).trim()) {
          t += '<p style="margin:6pt 0 0;font-size:9pt;color:#374151"><strong>Notes:</strong> ' + esc(card.notes) + '</p>';
        }
        t += '</div>';
      });
      return t;
    ...
    var html = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word">' +
      ...
      '</style></head><body>' +
      '<h1>Analysis Framework: ' + title + '</h1>' +
      ...
      '<p class="footer">Generated by PRAXIS Evaluation Workbench, ' + new Date().toISOString().slice(0, 10) + '</p>' +
      '</body></html>';

    var blob = new Blob([html], { type: 'application/msword' });
    var filename = window.PraxisExportUtils.exportFilename(programmeName, 'analysis-framework', 'doc');
    if (typeof PraxisUtils !== 'undefined' && PraxisUtils.downloadBlob) {
      PraxisUtils.downloadBlob(blob, filename);
    } else {
